import { NeoModal } from '../../../neo_modal/src/js/modal/modal';
import { parents, parentsOne, getParentsUntil, unwrapElement, wrapElement, wrapLiContents } from './utils/dom';
...
class SlideMenu {
  ...
  private initSubmenus() {
    ...
    this.menuElem.querySelectorAll('a').forEach((anchor: HTMLAnchorElement) => {
      ...
      // const parents = getParentsUntil(anchor.parentElement, 'ul', '.' + CLASS_NAMES.wrapper);
      // anchor.parentElement.classList.add(CLASS_NAMES.level + parents.length);
...
(function (Drupal) {
  ...
  Drupal.behaviors.neoSlideMenu = {
    ...
    modalTitle: (modal:NeoModal, title:HTMLElement) => {
      ...
      if (menu) {
        ...
        if (instance && instance.focusElem) {
          ...
          // titleElm.classList.add('whitespace-nowrap', 'overflow-hidden', 'text-ellipsis');
